 ...
 * Gillespie's @f$\tau@f$-leap algorithm. This is a discrete time Markov chain
 * whose states are given by the number of molecules of @f$\ell@f$ chemical 
 * species @f$S_1,S_2,\dots,S_{\ell}@f$. The molecules can react  
 * via @f$K@f$ predefined chemical reactions @f$R_1,R_2,\dots,R_K@f$
 * over a period of time @f$[0,T]@f$. The time interval is subdivided
...
public abstract class ChemicalReactionNetwork extends MarkovChainComparable implements MultiDim {
	 ...
	 */
	double[][] S; //rows: species, columns: reactions (check to be 100 % sure!)
	/**
	 * Reaction rates.
	 */
	double[] c;
	...
	/**
	* Getter for number of reactions
	*/
	public int getK() {
		return K;
	}
	...
	/**
	* (Re)sets the state X to its initial value X0
	*/
...
//		states = new double[N];
		...
		while (step < numSteps && !hasStopped()) {
//			states = new double[getPoint().length];
			nextStep(stream, states, reps);
			++step;
		}
	}
...
//			states[i] = new double[];
	 ...
	 * Simulates \a n chains over \a numSteps steps using the #nextStep method that
	 * repeats each step \a reps times and takes the average of these repetitions as
	 * the future state. The data is displayed and written to a csv-file at the
	 * location \a dataPath.
	 * 
...
//		for (int step = numSteps-1; step < numSteps; step++) {
