 ...
 * Baker '18. Since the total number of molecules is not change by the
 * reactions, the state space is, in fact, one dimensional.
 * 
 * @author florian
 *
 */
public class ReversibleIsomerization extends ChemicalReactionNetwork implements MultiDim {
	...
	public int compareTo(MarkovChainComparable m, int i) {
		if (!(m instanceof ReversibleIsomerization)) {
			throw new IllegalArgumentException("Can't compare a ReversibleIso with other types of Markov chains.");
		}
		double mx;
